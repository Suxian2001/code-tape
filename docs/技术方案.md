...
5. 导出量化：优先导出 Transformers.js 兼容 ONNX，并提供 q4/q8 量化版本；浏览器运行时优先尝试 q4，若 ONNX Runtime Web 报出 `TransposeDQWeightsForMatMulNBits`、`MatMulNBits` 或缺少 scale 等量化权重兼容性错误，必须自动回退到 q8 重新加载；若 q8 仍失败，则保留原始 ASR 字幕并展示可恢复错误，不在前端吞异常继续展示“已纠错”。离线导出侧仍应回退到未融合或重新导出的权重修复根因。
