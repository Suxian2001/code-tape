...
LLM 用于“ASR 后处理”，不替代 Whisper/ASR。默认不依赖项目专属微调模型，而是直接使用公开、Transformers.js 兼容、对中文和代码术语更友好的通用指令模型。它只消费已经带时间戳的字幕段和代码讲解上下文，输出两个独立派生结果：
...
- 选择原因：该仓库标记为 Transformers.js / ONNX / text-generation，浏览器本地推理只拉取公开模型资产，不需要也不得携带 Hugging Face token；Qwen2.5 Instruct 对中文、英文、代码术语和结构化指令输出的通用能力优于更小的英文倾向模型，适合“简体中文 + 前端术语 + JSON”后处理场景。
...
项目专属微调不作为默认路线。后续如确有大量已授权样本，可把 LoRA/PEFT 微调作为离线增强实验，训练、评估、量化和发布不在浏览器中完成，且必须先用 `npm run subtitle:dataset:validate` 和 `npm run subtitle:evaluate` 证明 JSON 合法率、稀疏 segment 覆盖、非空章节监督率、简体中文一致性和术语保真率确有提升，再讨论是否替换默认模型。
