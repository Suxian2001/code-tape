...
| `content-change` | editor | 是 | 合并后的完整代码、语言、change reason、flush reason |
...
- 如果 flush 时 `code` 与上一条稳定代码完全一致，不写入新事件，只更新选区或滚动事件。
- 大段粘贴或格式化只生成一条 `content-change`，`reason` 标为 `paste` / `format`，`changeCount` 记录合并次数。
