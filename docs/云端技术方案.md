...
- `events`、`snapshots`、`meta`、`manifest`、`indexes` 继续使用现有 schema。
...
- `manifest`、`meta`、`indexes` 可直接由 API 返回或签名下载。
...
stateDiagram-v2
  ...
  ready --> deleting: deleteRequested
  deleting --> deleted: objectsSwept
```
...
| `status`           | enum              | `draft/uploading/processing/ready/failed/deleting/deleted` |
| `visibility`       | enum              | `private/unlisted`                                         |
| `created_at`       | datetime          | 创建时间                                                   |
| `updated_at`       | datetime          | 更新时间                                                   |
| `completed_at`     | datetime nullable | 校验完成时间                                               |
| `deleted_at`       | datetime nullable | 软删时间                                                   |
...
| `indexes.json`   | `application/json` | 可长缓存，可通过版本化 key 更新 |
...
4. `CloudPackageLoader` 下载 `manifest`、`meta`、`events`、`snapshots`、`indexes`。
...
- 只有 owner 可以删除。
- API 先把状态置为 `deleting`，列表和播放立即不可见。
- Worker 异步删除对象存储资产、分享链接和派生缩略图。
- 删除完成后状态置为 `deleted`。
- 若对象存储删除失败，保留 `deleting` 状态并重试。

P1 默认软删 7 天可恢复，是否提供恢复 UI 由产品确认；如果不做恢复 UI，也建议保留短期软删以降低误删风险。
...
- 必需资产：`manifest`、`meta`、`events`、`snapshots`、`indexes`。
...
5. 读取并校验 `meta.json`、`events.json`、`snapshots.json`、`indexes.json`。
6. 执行 `validateRecordingPackageV1()`。
7. 若 schema 可迁移，执行 `migrateRecordingPackage()` 并生成新版本派生资产。
8. 校验 `seq` 单调、`timestampMs` 不倒退、`durationMs` 合理。
9. 校验快照边界：`snapshot.eventSeq` 必须能对应事件 seq。
10. 校验媒体：size、mimeType、duration 允许误差、`timelineOffsetMs`。
11. 更新数据库字段和状态。
...
前端上传的 `indexes.json` 可以直接使用，但服务端仍要能够重建核心索引：
...
P1 活动密度时间轴建议由服务端异步生成，避免列表和播放初始加载阻塞。
...
- `DELETE /recordings/:id`：重复删除返回 deleting 或 deleted。
...
- 删除时 recording 从 `ready` 到 `deleting`。
...
| deleting 清理     | 每 5 分钟  | 删除对象存储资产，状态置 deleted                     |
...
| indexes 缺失    | 前端从 events/snapshots 重建 `ReplayIndex` |
| 缩略图缺失      | 使用语言和时长占位                         |
...
- 首屏先加载 recording detail 和 indexes。
...
| 删除对象失败                  | 存储泄露和成本增加               | `deleting` 状态重试、孤儿对象扫描               |
